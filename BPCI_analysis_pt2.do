...
table year medicare, c(mean los_w)
table year medicare, c(mean postcare)
table year medicare, c(sum age3)
tabstat totchg_r totchg_w totchg_rlog, by(year) stat(mean sem)
tabstat los_r los_w los_rlog, by(year) stat(mean sem)
tabstat teach snf_ rehab_ ltc_, by(year) stat(count mean sem)
tabstat totchg_w los_w postcare female highsev dual icu racecat percenturban median_householdincome bedsize hcontrl teach snf_ rehab_ ltc_ totaldisc, stat(mean sd min max) col(stat)
...
tab ethnicity, m
tab racecat, m
table year racecat, c(mean totchg_w)
table year racecat, c(mean los_w)
table year racecat, c(mean postcare)
table year insurance, c(mean totchg_w)
table year insurance, c(mean los_w)
table year insurance, c(mean postcare)
table year agecat, c(mean totchg_w)
table year agecat, c(mean los_w)
table year agecat, c(mean postcare)

table year medicare, c(mean teach)
table year medicare, c(mean snf_)
table year medicare, c(mean rehab_)
table year medicare, c(mean ltc_)

table year medicare if inlist(drg, 469, 470), c(mean totchg_w)
table year medicare if inlist(drg, 391, 392), c(mean totchg_w)
table year medicare if inlist(drg, 870, 871, 872), c(mean totchg_w)
table year medicare if inlist(drg, 469, 470), c(mean los_w)
table year medicare if inlist(drg, 391, 392), c(mean los_w)
table year medicare if inlist(drg, 870, 871, 872), c(mean los_w)
table year medicare if inlist(drg, 469, 470), c(mean postcare)
table year medicare if inlist(drg, 391, 392), c(mean postcare)
table year medicare if inlist(drg, 870, 871, 872), c(mean postcare)


**look at facility characteristics

table bpci, c(m  icu m urban m income m bedsize m hcontr)
...
table year medicare, c(mean totchg_r)
table year medicare, c(mean los_r)
...
reg totchg_r medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg totchg_r medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg totchg_r medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg totchg_w medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg totchg_w medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg totchg_w medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg totchg_rlog medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg totchg_rlog medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg totchg_rlog medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg los_r medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg los_r medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg los_r medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg los_w medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg los_w medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg los_w medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg los_rlog medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg los_rlog medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg los_rlog medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
logit postcare medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, or vce(cluster thcic_id)
...
logit postcare medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, or vce(cluster thcic_id)
logit postcare medicare post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, or vce(cluster thcic_id)
...
codebook record_id
table year bpci, c(mean totchg_w)
table year bpci, c(mean los_w)
table year bpci, c(mean postcare)
tabstat totchg_r totchg_w totchg_rlog, by(year) stat(mean sem)
tabstat los_r los_w los_rlog, by(year) stat(mean sem)
tabstat teach snf_ rehab_ ltc_, by(year) stat(count mean sem)
tabstat totchg_w los_w postcare female highsev dual icu ethnicity racecat percenturban median_householdincome bedsize hcontrl teach snf_ rehab_ ltc_ totaldisc, stat(mean sd min max) col(stat)
tabstat los_w totchg_w postcare, by(year) stat(mean sem min max)
tabstat los_w totchg_w postcare, by(year) stat(mean sem min max)
...
table year bpci, c(mean teach)
table year bpci, c(mean snf_)
table year bpci, c(mean rehab_)
table year bpci, c(mean ltc_)

table year bpci if inlist(drg, 469, 470), c(mean totchg_w)
table year bpci if inlist(drg, 391, 392), c(mean totchg_w)
table year bpci if inlist(drg, 870, 871, 872), c(mean totchg_w)
table year bpci if inlist(drg, 469, 470), c(mean los_w)
table year bpci if inlist(drg, 391, 392), c(mean los_w)
table year bpci if inlist(drg, 870, 871, 872), c(mean los_w)
table year bpci if inlist(drg, 469, 470), c(mean postcare)
table year bpci if inlist(drg, 391, 392), c(mean postcare)
table year bpci if inlist(drg, 870, 871, 872), c(mean postcare)
...
table year bpci, c(mean totchg_r)
table year bpci, c(mean los_r)
...
reg totchg_r bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg totchg_r bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg totchg_r bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg totchg_w bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg totchg_w bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg totchg_w bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg totchg_rlog bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg totchg_rlog bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg totchg_rlog bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg los_r bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg los_r bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg los_r bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg los_w bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg los_w bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg los_w bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
reg los_rlog bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, vce(cluster thcic_id)
...
reg los_rlog bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, vce(cluster thcic_id)
reg los_rlog bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, vce(cluster thcic_id)
...
logit postcare bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if snf_==1, or vce(cluster thcic_id)
...
logit postcare bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if rehab_==1, or vce(cluster thcic_id)
logit postcare bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year if ltc_==1, or vce(cluster thcic_id)
...
reg postcare bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year  if snf_==1,   vce(cluster thcic_id)
...
reg postcare bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year  if rehab_==1,   vce(cluster thcic_id)
reg postcare bpci post did female highsev dual icu ethnicity i.racecat i.urban i.income i.bedsize i.hcontrl i.year  if ltc_==1,   vce(cluster thcic_id)
